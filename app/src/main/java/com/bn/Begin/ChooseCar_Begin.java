...
public class ChooseCar_Begin extends Activity{
	...
	//public static int ChooseCar=0;
	
	public static String ChooseCar_Begin_Get;//从最开始进入后 所得到的题库的String值
	public static int ChooseCar_Begin=0;//从最开始选车界面的标志位
			...
			public void handleMessage(Message msg) 
	   		{
	   			...
	   			switch(msg.what)
	   			{
					 ...
					 case ResetConstant.TESTCONNECTERROR://测试连接失败1
						 	 NetIsOk_Car=false;
							 Toast.makeText(ChooseCar_Begin.this,"连接服务器失败,请打开WiFi连接服务器.",Toast.LENGTH_SHORT).show();
					 break;
	   			}
	   		}
		};
	...
	protected void onCreate(Bundle savedInstanceState)
	{
	    ...
	    button1.setOnTouchListener(new View.OnTouchListener()
	    {
			...
			else if(event.getAction() == MotionEvent.ACTION_UP)
			{
				...
				{
					Toast.makeText(ChooseCar_Begin.this,"暂时未能连接到服务器，请您检查您的网络连接",Toast.LENGTH_SHORT).show();
				}
				else
				{
					//设置题库 等待3s后
					final CustomProgressDialog dialog =new CustomProgressDialog(ChooseCar_Begin.this, "正在获取试题信息...",R.anim.frame);
	    ...
	    button2.setOnTouchListener(new View.OnTouchListener()
	    {
			...
			else if(event.getAction() == MotionEvent.ACTION_UP)
			{
				...
				{
					//设置题库 等待3s后跳转
					final CustomProgressDialog dialog =new CustomProgressDialog(ChooseCar_Begin.this, "客官，请稍等...",R.anim.frame);
					//计时后结束 并跳转
					Handler handler = new Handler(); 
			        handler.postDelayed(new Runnable() { 
	    ...
	    buttonOk.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
						...
						KeYun_Down==1||HuoYun_Down==1||Wei_Down==1||Jiao_Down==1)
				{
					if(Car_Down==1)
					{
						ChooseCar_Begin_Get="小车";
					}
					else if(Huo_Down==1)
					{
						ChooseCar_Begin_Get="货车";
					}
					else if(Ke_Down==1)
					{
						ChooseCar_Begin_Get="客车";
					}
					else if(Mo_Down==1)
					{
						ChooseCar_Begin_Get="摩托车";
					}
					else if(KeYun_Down==1)
					{
						ChooseCar_Begin_Get="客运";
					}
					else if(HuoYun_Down==1)
					{
						ChooseCar_Begin_Get="货运";
					}
					else if(Wei_Down==1)
					{
						ChooseCar_Begin_Get="危险品";
					}
					else 
					{
						ChooseCar_Begin_Get="教练";
					}
					
					DBUtil.deleteCar();//删除
					DBUtil.insertCar(ChooseCar_Begin_Get);//插入 
					CustomProgressDialog dialog =new CustomProgressDialog(ChooseCar_Begin.this, "正在设置题库中...",R.anim.frame);
					//计时后结束 并跳转
					Handler handler = new Handler(); 
			        handler.postDelayed(new Runnable() { 
			            ...
			            public void run() { 
			            	...
			            	intent.setClass(ChooseCar_Begin.this,SchoolList.class);
			        		ChooseCar_Begin=1;//标志位设为1 
